...
func WriteIni(section string, key string, value string) bool {
	if IniParserConf.conf_reader == nil {
		return false
	}

	IniParserConf.conf_reader.Section(section).Key(key).SetValue(value)
	err := IniParserConf.conf_reader.SaveTo(ConfigFile)
	if err != nil {
		log.Printf("write config.ini failed. section: %s, key: %s, value: %s", section, key, value)
		return false
	}
	log.Printf("write config.ini successfully. section: %s, key: %s, value: %s", section, key, value)
